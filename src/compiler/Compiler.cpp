...
namespace pycc {
    ...
    buildModuleGroup(const std::filesystem::path &moduleFile,
        ...
        for (size_t i = 0; i < toks.size(); ++i) {
            const auto &t = toks[i];
            if (t.kind == lex::TokenKind::Indent) {
                ++indentDepth;
                continue;
            }
            if (t.kind == lex::TokenKind::Dedent) {
            ...
            if (t.kind == lex::TokenKind::Import) {
                ...
                while (i < toks.size()) {
                    ...
                    dotted = toks[i].text;
                    ++i;
                    while (i + 1 < toks.size() && toks[i].kind == lex::TokenKind::Dot && toks[i + 1].kind ==
                           lex::TokenKind::Ident) {
                        ++i;
                        dotted += ".";
                        dotted += toks[i].text;
                    ...
                    // optional 'as alias'
                    if (i + 1 < toks.size() && toks[i].kind == lex::TokenKind::As && toks[i + 1].kind ==
                        lex::TokenKind::Ident) { i += 2; }
                    // resolve
                    if (!dotted.empty()) {
                    ...
                    if (i < toks.size() && toks[i].kind == lex::TokenKind::Comma) {
                        ++i;
                        continue;
                    }
                    break;
                ...
                int level = 0;
                size_t j = i + 1;
                while (j < toks.size() && (toks[j].kind == lex::TokenKind::Dot || toks[j].kind ==
                ...
                if (j < toks.size() && toks[j].kind == lex::TokenKind::Ident) {
                    module = toks[j].text;
                    ++j;
                    while (j + 1 < toks.size() && toks[j].kind == lex::TokenKind::Dot && toks[j + 1].kind ==
                           lex::TokenKind::Ident) {
                        ++j;
                        module += ".";
                        module += toks[j].text;
                ...
                // expect 'import'
                while (j < toks.size() && toks[j].kind != lex::TokenKind::Import && toks[j].kind !=
                       lex::TokenKind::Newline) { ++j; }
                if (j >= toks.size() || toks[j].kind != lex::TokenKind::Import) {
                    i = j;
                    continue;
                }
                ++j;
                ...
                while (j < toks.size()) {
                    ...
                    ++j;
                    if (j + 1 < toks.size() && toks[j].kind == lex::TokenKind::As && toks[j + 1].kind ==
                        lex::TokenKind::Ident) { j += 2; }
                    ...
                    if (j < toks.size() && toks[j].kind == lex::TokenKind::Comma) {
                        ++j;
                        continue;
                    }
                    break;
    ...
    int Compiler::run(const cli::Options &opts) { // NOLINT(readability-function-size,readability-function-cognitive-complexity)
        ...
        try {
            ...
            bool color = false;
            if (opts.color == cli::ColorMode::Always) { color = true; } else if (opts.color == cli::ColorMode::Never) {
                color = false;
            } else {
        ...
        if (opts.optConstFold) {
            ...
            for (const auto &[key, count]: constFold.stats()) {
                metrics.incOptimizerBreakdown("constfold", key, count);
            }
        ...
        if (opts.optAlgebraic) {
            ...
            for (const auto &[key, count]: algebraic.stats()) {
                metrics.incOptimizerBreakdown("algebraic", key, count);
            }
        ...
        auto recordPost = [&]() {
            ...
            try {
                ...
                uint64_t lines = 0;
                for (const char c: irText) if (c == '\n') ++lines;
                metrics.setGauge("codegen.ir_lines", lines);
            } catch (const std::exception &ex) { (void) ex; }
        };
        ...
        if (opts.metrics || opts.metricsJson) {
            ...
            metrics.setGauge("rt.alloc_rate_bps",
                             static_cast<uint64_t>(telem.allocRateBytesPerSec >= 0 ? telem.allocRateBytesPerSec : 0.0));
            metrics.setGauge("rt.pressure_ppm",
                             static_cast<uint64_t>((telem.pressure >= 0 ? telem.pressure : 0.0) * 1000000.0));
        }
        ...
        if (logsEnabled) {
            ...
            if (opts.logLexer) {
                ...
                for (const auto &tok: lexer.tokens()) {
                    lexFile << tok.file << ":" << tok.line << ":" << tok.col << " " << to_string(tok.kind) << " " << tok
                            .text << "\n";
                }
            }
            // Codegen log (IR)
