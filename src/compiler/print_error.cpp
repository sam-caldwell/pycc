...
static void print_header(const sema::Diagnostic& diag, bool color) {
...
static void print_label(bool color) {
  if (color) { write_str(kRed); write_str("error: "); write_str(kReset); }
...
static void print_context(int context) {
  if (context <= 0) { return; }
