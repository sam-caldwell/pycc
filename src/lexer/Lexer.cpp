...
namespace pycc::lex {
    static bool isIdentStart(const char chr) {
        return (std::isalpha(static_cast<unsigned char>(chr)) != 0) || chr == '_';
    }

    static bool isIdentChar(const char chr) {
...
#include <unicode/uchar.h>
    static bool decodeUtf8Cp(const std::string &s, size_t i, UChar32 &out, size_t &next) {
        ...
        if ((c0 & 0xE0U) == 0xC0U && i + 1 < s.size()) {
            out = ((c0 & 0x1FU) << 6) | (static_cast<unsigned char>(s[i + 1]) & 0x3FU);
            next = i + 2;
            return true;
        }
        if ((c0 & 0xF0U) == 0xE0U && i + 2 < s.size()) {
            out = ((c0 & 0x0FU) << 12) | ((static_cast<unsigned char>(s[i + 1]) & 0x3FU) << 6) | (
                      static_cast<unsigned char>(s[i + 2]) & 0x3FU);
            next = i + 3;
            return true;
        }
        if ((c0 & 0xF8U) == 0xF0U && i + 3 < s.size()) {
            out = ((c0 & 0x07U) << 18) | ((static_cast<unsigned char>(s[i + 1]) & 0x3FU) << 12) | (
    ...
    bool Lexer::emitIndentTokens(State &state, std::vector<Token> &out, const int baseCol) {
        ...
        size_t idx = 0;
        size_t spaces = 0;
        while (idx < state.line.size() && state.line[idx] == ' ') {
            ++idx;
            ++spaces;
        }
        // Empty or comment line: emit Newline and skip indent changes
        const bool allSpace = (idx >= state.line.size());
        if (const bool comment = (!allSpace && state.line[idx] == '#'); allSpace || comment) {
    ...
    Token Lexer::scanOne(State &state) {
        ...
        if (idx >= line.size()) {
            ...
            newlineTok.col = static_cast<int>(line.size() + 1);
            return newlineTok;
        }
        ...
        if (chr == '(') {
            ++idx;
            return makeTok(TokenKind::LParen, idx - 1, idx);
        }
        if (chr == ')') {
            ++idx;
            return makeTok(TokenKind::RParen, idx - 1, idx);
        }
        if (chr == '[') {
            ++idx;
            return makeTok(TokenKind::LBracket, idx - 1, idx);
        }
        if (chr == ']') {
            ++idx;
            return makeTok(TokenKind::RBracket, idx - 1, idx);
        }
        if (chr == '{') {
            ++idx;
            return makeTok(TokenKind::LBrace, idx - 1, idx);
        }
        if (chr == '}') {
            ++idx;
            return makeTok(TokenKind::RBrace, idx - 1, idx);
        }
        if (chr == ':') {
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::ColonEqual, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Colon, idx - 1, idx);
        }
        if (chr == ',') {
            ++idx;
            return makeTok(TokenKind::Comma, idx - 1, idx);
        }
        if (chr == '@') {
            ++idx;
            return makeTok(TokenKind::At, idx - 1, idx);
        }
        if (chr == '+') {
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::PlusEqual, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Plus, idx - 1, idx);
        }
        auto scanStringLike = [&](size_t start, bool hasB, bool hasR, bool hasF) -> Token {
            ...
            while (p < line.size() && (line[p] == 'b' || line[p] == 'B' || line[p] == 'f' || line[p] == 'F' || line[p]
                                       == 'r' || line[p] == 'R' || line[p] == 'u' || line[p] == 'U')) { ++p; }
            if (p >= line.size()) {
                Token t = makeTok(hasB ? TokenKind::Bytes : TokenKind::String, start, p);
            ...
            bool triple = false;
            if (p + 2 < line.size() && (line[p] == line[p + 1]) && (line[p] == line[p + 2]) && (
                    quote == '"' || quote == '\'')) { triple = true; }
            size_t endPos = p + 1;
            if (!triple) {
                bool escape = false;
        ...
        if ((chr == 'b' || chr == 'B' || chr == 'f' || chr == 'F' || chr == 'r' || chr == 'R' || chr == 'u' || chr ==
             'U') && idx + 1 < line.size()) {
            size_t p = idx;
            bool hasB = false;
            bool hasR = false;
            ...
            while (cnt < 2 && p < line.size()) {
                const char c = line[p];
                if (c == 'b' || c == 'B') { hasB = true; } else if (c == 'r' || c == 'R') { hasR = true; } else if (
                    c == 'f' || c == 'F') { hasF = true; } else if (c == 'u' || c == 'U') {
                    /* legacy unicode prefix tolerated */
                } else break;
                ++p;
                ++cnt;
            }
            if (p < line.size() && (line[p] == '\'' || line[p] == '"')) {
                return scanStringLike(idx, hasB && !hasF, hasR, hasF);
            }
            if (p + 2 < line.size() && line[p] == line[p + 1] && line[p] == line[p + 2] && (
                    line[p] == '\'' || line[p] == '"')) { return scanStringLike(idx, hasB && !hasF, hasR, hasF); }
        }
        if (chr == '-') {
            if (idx + 1 < line.size() && line[idx + 1] == '>') {
                idx += 2;
                return makeTok(TokenKind::Arrow, idx - 2, idx);
            }
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::MinusEqual, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Minus, idx - 1, idx);
        }
        if (chr == '*') {
            if (idx + 1 < line.size() && line[idx + 1] == '*') {
                if (idx + 2 < line.size() && line[idx + 2] == '=') {
                    idx += 3;
                    return makeTok(TokenKind::StarStarEqual, idx - 3, idx);
                }
                idx += 2;
                return makeTok(TokenKind::StarStar, idx - 2, idx);
            }
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::StarEqual, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Star, idx - 1, idx);
        }
        if (chr == '/') {
            if (idx + 1 < line.size() && line[idx + 1] == '/') {
                if (idx + 2 < line.size() && line[idx + 2] == '=') {
                    idx += 3;
                    return makeTok(TokenKind::SlashSlashEqual, idx - 3, idx);
                }
                idx += 2;
                return makeTok(TokenKind::SlashSlash, idx - 2, idx);
            }
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::SlashEqual, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Slash, idx - 1, idx);
        }
        if (chr == '%') {
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::PercentEqual, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Percent, idx - 1, idx);
        }
        if (chr == '=') {
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::EqEq, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Equal, idx - 1, idx);
        }
        if (chr == '!') {
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::NotEq, idx - 2, idx);
            }
        }
        if (chr == '<') {
            if (idx + 1 < line.size() && line[idx + 1] == '<') {
                if (idx + 2 < line.size() && line[idx + 2] == '=') {
                    idx += 3;
                    return makeTok(TokenKind::LShiftEqual, idx - 3, idx);
                }
                idx += 2;
                return makeTok(TokenKind::LShift, idx - 2, idx);
            }
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::Le, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Lt, idx - 1, idx);
        }
        if (chr == '>') {
            if (idx + 1 < line.size() && line[idx + 1] == '>') {
                if (idx + 2 < line.size() && line[idx + 2] == '=') {
                    idx += 3;
                    return makeTok(TokenKind::RShiftEqual, idx - 3, idx);
                }
                idx += 2;
                return makeTok(TokenKind::RShift, idx - 2, idx);
            }
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::Ge, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Gt, idx - 1, idx);
        }
        if (chr == '|') {
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::PipeEqual, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Pipe, idx - 1, idx);
        }
        if (chr == '&') {
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::AmpEqual, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Amp, idx - 1, idx);
        }
        if (chr == '^') {
            if (idx + 1 < line.size() && line[idx + 1] == '=') {
                idx += 2;
                return makeTok(TokenKind::CaretEqual, idx - 2, idx);
            }
            ++idx;
            return makeTok(TokenKind::Caret, idx - 1, idx);
        }
        if (chr == '~') {
            ++idx;
            return makeTok(TokenKind::Tilde, idx - 1, idx);
        }
        ...
        auto scanExponent = [&](size_t pos) -> size_t {
            ...
            if (idx < line.size() && (line[idx] == 'e' || line[idx] == 'E')) {
                ...
                while (idx < line.size()) {
                    ...
                    if (d == '_' && digits > 0 && !prevUnderscore) {
                        prevUnderscore = true;
                        ++idx;
                        continue;
                    }
        ...
        if (std::isdigit(static_cast<unsigned char>(chr)) != 0) {
            ...
            if (line[idx] == '0' && idx + 1 < line.size()) {
                ...
                if (p1 == 'b' || p1 == 'B' || p1 == 'o' || p1 == 'O' || p1 == 'x' || p1 == 'X') {
                    ...
                    size_t end = p;
                    if (end < line.size() && (line[end] == 'j' || line[end] == 'J')) {
                        ++end;
                        Token tok = makeTok(TokenKind::Imag, i0, end);
                        idx = end;
                        return tok;
                    }
                    Token tok = makeTok(TokenKind::Int, i0, p);
                    idx = p;
                    return tok;
                }
            }
            ...
            // Float with a fractional part or dot-only fractional
            if (p < line.size() && line[p] == '.') {
                ...
                Token tok = makeTok(TokenKind::Float, i0, epos);
                idx = epos;
                return tok;
            }
            ...
            if (epos != p) {
                if (size_t end = epos; end < line.size() && (line[end] == 'j' || line[end] == 'J')) {
                    ++end;
                    Token tok = makeTok(TokenKind::Imag, i0, end);
                    idx = end;
                    return tok;
        ...
        if (chr == '.') {
            if (idx + 2 < line.size() && line[idx + 1] == '.' && line[idx + 2] == '.') {
                idx += 3;
                return makeTok(TokenKind::Ellipsis, idx - 3, idx);
            }
            if (idx + 1 < line.size() && std::isdigit(static_cast<unsigned char>(line[idx + 1])) != 0) {
                size_t mpos = idx + 1;
                // scan digits with underscores
                bool prevUnderscore = false;
                while (mpos < line.size()) {
                    const char c = line[mpos];
                    if (std::isdigit(static_cast<unsigned char>(c)) != 0) {
                        prevUnderscore = false;
                        ++mpos;
                    } else if (c == '_' && !prevUnderscore) {
                        prevUnderscore = true;
                        ++mpos;
                    } else break;
                }
                if (prevUnderscore) --mpos;
                const size_t epos = scanExponent(mpos);
                size_t end = epos;
                if (end < line.size() && (line[end] == 'j' || line[end] == 'J')) {
                    ++end;
                    const Token tok = makeTok(TokenKind::Imag, idx, end);
                    idx = end;
                    return tok;
                }
            ...
            ++idx;
            return makeTok(TokenKind::Dot, idx - 1, idx);
        }
        ...
        if (
            ...
            if (static_cast<unsigned char>(chr) >= 0x80) {
                ...
                UChar32 cp = 0;
                size_t next = idx;
                if (!decodeUtf8Cp(line, idx, cp, next) || !isXIDStartCp(cp)) {
                    ++idx;
                    return makeTok(TokenKind::Unknown, idx - 1, idx);
                }
                jpos = next;
                while (jpos < line.size()) {
                    UChar32 cp2 = 0;
                    size_t nx = 0;
                    if (!decodeUtf8Cp(line, jpos, cp2, nx)) break;
                    if (!isXIDContinueCp(cp2)) break;
                    jpos = nx;
                }
            } else {
            ...
            if (ident == "def") { kind = TokenKind::Def; } else if (
                ident == "return") { kind = TokenKind::Return; } else if (
                ident == "del") { kind = TokenKind::Del; } else if (ident == "if") { kind = TokenKind::If; } else if (
                ident == "else") { kind = TokenKind::Else; } else if (ident == "elif") { kind = TokenKind::Elif; } else
                if (ident == "while") { kind = TokenKind::While; } else if (
                    ident == "for") { kind = TokenKind::For; } else if (ident == "in") { kind = TokenKind::In; } else if
                (ident == "break") { kind = TokenKind::Break; } else if (
                    ident == "continue") { kind = TokenKind::Continue; } else if (
                    ident == "pass") { kind = TokenKind::Pass; } else if (
                    ident == "try") { kind = TokenKind::Try; } else if (
                    ident == "except") { kind = TokenKind::Except; } else if (
                    ident == "finally") { kind = TokenKind::Finally; } else if (
                    ident == "with") { kind = TokenKind::With; } else if (ident == "as") { kind = TokenKind::As; } else
                    if (ident == "match") { kind = TokenKind::Match; } else if (
                        ident == "case") { kind = TokenKind::Case; } else if (
                        ident == "import") { kind = TokenKind::Import; } else if (
                        ident == "from") { kind = TokenKind::From; } else if (
                        ident == "class") { kind = TokenKind::Class; } else if (
                        ident == "async") { kind = TokenKind::Async; } else if (
                        ident == "assert") { kind = TokenKind::Assert; } else if (
                        ident == "raise") { kind = TokenKind::Raise; } else if (
                        ident == "global") { kind = TokenKind::Global; } else if (
                        ident == "nonlocal") { kind = TokenKind::Nonlocal; } else if (
                        ident == "yield") { kind = TokenKind::Yield; } else if (
                        ident == "await") { kind = TokenKind::Await; } else if (
                        ident == "and") { kind = TokenKind::And; } else if (
                        ident == "or") { kind = TokenKind::Or; } else if (
                        ident == "not") { kind = TokenKind::Not; } else if (
                        ident == "lambda") { kind = TokenKind::Lambda; } else if (
                        ident == "is") { kind = TokenKind::Is; } else if (
                        ident == "True" || ident == "False") { kind = TokenKind::BoolLit; } else if (
                        ident == "int" || ident == "bool" || ident == "float" || ident == "str" || ident == "None" ||
                        ident == "tuple" || ident == "list" || ident == "dict" || ident == "Optional" || ident ==
                        "Union") { kind = TokenKind::TypeIdent; }
    ...
    void Lexer::buildAll() {
        ...
        while (!stack_.empty()) {
            ...
            while (readNextLine(state)) {
                ...
                for (auto const &tok: indents) { tokens_.push_back(tok); }
            ...
            while (state.indentStack.size() > 1) {
                state.indentStack.pop_back();
                Token ded;
                ded.kind = TokenKind::Dedent;
                ded.text = "<DEDENT>";
                ded.file = state.src->name();
                ded.line = state.lineNo + 1;
                ded.col = 1;
