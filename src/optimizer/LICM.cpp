...
static void collectReadNames(const Expr* e, std::unordered_set<std::string>& reads) {
  ...
  switch (e->kind) {
    ...
    case NodeKind::Unary: collectReadNames(static_cast<const Unary*>(e)->operand.get(), reads); break;
    case NodeKind::Binary: collectReadNames(static_cast<const Binary*>(e)->lhs.get(), reads); collectReadNames(static_cast<const Binary*>(e)->rhs.get(), reads); break;
