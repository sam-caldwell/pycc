...
void ExpressionTyper::visit(const ast::Name &n) {
    ...
    if (maskVal == 0U && pycc::sema::detail::g_locals_assigned && pycc::sema::detail::g_locals_assigned->contains(n.id)) {
    ...
    if (maskVal == 0U && outers != nullptr) {
        for (const auto *o: *outers) {
            if (!o) continue;
            const uint32_t m = o->getSet(n.id);
            if (m != 0U) {
                maskVal = m;
                break;
            }
    ...
    if (maskVal == 0U) {
        ...
        if (outers != nullptr) {
            ...
            if (outerMask != 0U) {
                outSet = outerMask;
                if (TypeEnv::isSingleMask(outerMask)) out = TypeEnv::kindFromMask(outerMask);
                n.setType(out);
    ...
    if (const auto resolvedType = env->get(n.id); !resolvedType && outSet == 0U) {
        addDiag(*diags, std::string("undefined name: ") + n.id, &n);
        ok = false;
        return;
    }
