...
// Removed file-local helpers to maintain one function per .cpp
...
void ExpressionTyper::visit(const ast::Call &callNode) {
    ...
    if (callNode.callee && callNode.callee->kind == ast::NodeKind::Attribute) {
        ...
        if (const auto *at0 = static_cast<const ast::Attribute *>(callNode.callee.get());
            at0->value && at0->value->kind == ast::NodeKind::Name) {
            ...
            if (base0->id == "math") {
                ...
                auto checkUnary = [&](ast::TypeKind retKind) {
                    ...
                    const uint32_t okmask = TypeEnv::maskForKind(ast::TypeKind::Int) | TypeEnv::maskForKind(
                                                ast::TypeKind::Float);
                    if ((mask & ~okmask) != 0U) {
                        addDiag(*diags, std::string("math.") + fn + ": argument must be int/float",
                                callNode.args[0].get());
                        ok = false;
                        return;
                    }
                ...
                auto checkBinary = [&](ast::TypeKind retKind) {
                    ...
                    ExpressionTyper a0{*env, *sigs, *retParamIdxs, *diags, polyTargets};
                    callNode.args[0]->accept(a0);
                    if (!a0.ok) {
                        ok = false;
                        return;
                    }
                    ExpressionTyper a1{*env, *sigs, *retParamIdxs, *diags, polyTargets};
                ...
                if (fn == "sqrt" || fn == "fabs" || fn == "sin" || fn == "cos" || fn == "tan" || fn == "asin" || fn ==
                    "acos" || fn == "atan" || fn == "exp" || fn == "exp2" || fn == "log" || fn == "log2" || fn ==
                    "log10" || fn == "degrees" || fn == "radians") {
    ...
    if (callNode.callee && callNode.callee->kind == ast::NodeKind::Name) {
        ...
        if (nameNode->id == "eval" || nameNode->id == "exec") {
            if (callNode.args.size() != 1 || !callNode.args[0] || callNode.args[0]->kind !=
                ast::NodeKind::StringLiteral) {
                addDiag(*diags,
                        std::string(nameNode->id) + "() only accepts a compile-time literal string in this subset",
                        &callNode);
                ok = false;
                return;
            }
        ...
        if (nameNode->id == "len") {
            ...
            if (!(k == ast::TypeKind::Str || k == ast::TypeKind::List || k == ast::TypeKind::Tuple || k ==
                  ast::TypeKind::Dict)) {
        ...
        if (it == sigs->end()) {
            ...
            addDiag(*diags, std::string("unknown function: ") + nameNode->id, &callNode);
            ok = false;
            return;
        }
        ...
        if (!sig.full.empty()) {
            ...
            std::vector<size_t> posParamIdxs;
            size_t varargIdx = static_cast<size_t>(-1), kwvarargIdx = static_cast<size_t>(-1);
            posParamIdxs.reserve(sig.full.size());
            for (size_t i = 0; i < sig.full.size(); ++i) {
                const auto &p = sig.full[i];
                if (p.isVarArg) { varargIdx = i; } else if (p.isKwVarArg) { kwvarargIdx = i; } else {
                    nameToIdx[p.name] = i;
                    if (!p.isKwOnly) posParamIdxs.push_back(i);
                }
            }
            // Bound set
            ...
            if (!callNode.starArgs.empty()) {
                for (const auto &sa: callNode.starArgs) {
                    if (!sa) continue;
                    ExpressionTyper at{*env, *sigs, *retParamIdxs, *diags, polyTargets};
                    sa->accept(at);
                    if (!at.ok) {
                        ok = false;
                        return;
                    ...
                    if (at.out != ast::TypeKind::List) {
                        addDiag(*diags, "*args must be a list", sa.get());
                        ok = false;
                        return;
                    }
                    ...
                    if (varargIdx != static_cast<size_t>(-1) && sig.full[varargIdx].listElemMask != 0U) {
                        ...
                        if (sa->kind == ast::NodeKind::ListLiteral) {
                            ...
                            const auto *nm = static_cast<const ast::Name *>(sa.get());
                            elemMask = env->getListElems(nm->id);
                        }
                        if (elemMask != 0U && (elemMask & ~sig.full[varargIdx].listElemMask) != 0U) {
                            addDiag(*diags, "*args element type mismatch", sa.get());
                            ok = false;
                            return;
                        }
                    }
            ...
            for (size_t i = 0; i < callNode.args.size(); ++i) {
                ...
                if (i < posParamIdxs.size()) {
                    ...
                    if (at.out != sig.full[pidx].type) {
                        addDiag(*diags, "call argument type mismatch", callNode.args[i].get());
                        ok = false;
                        return;
                    }
                    bound[pidx] = true;
                } else if (varargIdx != static_cast<size_t>(-1)) {
                    if (sig.full[varargIdx].type != ast::TypeKind::NoneType && at.out != sig.full[varargIdx].type) {
                        addDiag(*diags, "*args element type mismatch", callNode.args[i].get());
                        ok = false;
                        return;
                    }
                } else {
                    addDiag(*diags, std::string("arity mismatch calling function: ") + nameNode->id, &callNode);
                    ok = false;
                    return;
                }
            }
            ...
            for (const auto &kw: callNode.keywords) {
                ...
                if (itn == nameToIdx.end()) {
                    if (kwvarargIdx == static_cast<size_t>(-1)) {
                        addDiag(*diags, std::string("unknown keyword argument: ") + kw.name, &callNode);
                        ok = false;
                        return;
                    }
                    continue;
                ...
                if (sig.full[pidx].isPosOnly) {
                    addDiag(*diags, std::string("positional-only argument passed as keyword: ") + kw.name, &callNode);
                    ok = false;
                    return;
                }
                if (bound[pidx]) {
                    addDiag(*diags, std::string("multiple values for argument: ") + kw.name, &callNode);
                    ok = false;
                    return;
                }
                ExpressionTyper kt{*env, *sigs, *retParamIdxs, *diags, polyTargets};
                if (kw.value) kw.value->accept(kt);
                if (!kt.ok) {
                    ok = false;
                    return;
                }
                ...
                bool typeOk = false;
                const uint32_t aMask = TypeEnv::maskForKind(kt.out);
                if (p.unionMask != 0U) { typeOk = ((aMask & p.unionMask) != 0U); } else if (
                    p.type == ast::TypeKind::List && p.listElemMask != 0U && kt.out == ast::TypeKind::List) {
                    uint32_t elemMask = 0U;
                    if (kw.value && kw.value->kind == ast::NodeKind::ListLiteral) {
                        ...
                        const auto *nm = static_cast<const ast::Name *>(kw.value.get());
                        elemMask = env->getListElems(nm->id);
                    }
                    if (elemMask != 0U) { typeOk = ((elemMask & ~p.listElemMask) == 0U); } else { typeOk = true; }
                } else { typeOk = (kt.out == p.type); }
                if (!typeOk) {
                    addDiag(*diags, std::string("keyword argument type mismatch: ") + kw.name, &callNode);
                    ok = false;
                    return;
                }
                bound[pidx] = true;
            }
            if (!callNode.starArgs.empty() && varargIdx == static_cast<size_t>(-1)) {
                addDiag(*diags, "*args provided but callee has no varargs", &callNode);
                ok = false;
                return;
            }
            if (!callNode.kwStarArgs.empty() && kwvarargIdx == static_cast<size_t>(-1)) {
                addDiag(*diags, "**kwargs provided but callee has no kwvarargs", &callNode);
                ok = false;
                return;
            }
            for (size_t i = 0; i < sig.full.size(); ++i) {
                const auto &sp = sig.full[i];
                if (sp.isVarArg || sp.isKwVarArg) continue;
                if (!bound[i] && !sp.hasDefault) {
                    if (sp.isKwOnly) {
                        addDiag(*diags, std::string("missing required keyword-only argument: ") + sp.name, &callNode);
                        ok = false;
                        return;
                    } else {
                        addDiag(*diags, std::string("missing required positional argument: ") + sp.name, &callNode);
                        ok = false;
                        return;
                    }
                }
            ...
            if (sig.params.size() != callNode.args.size()) {
                addDiag(*diags, std::string("arity mismatch calling function: ") + nameNode->id, &callNode);
                ok = false;
                return;
            }
            for (size_t i = 0; i < callNode.args.size(); ++i) {
                ExpressionTyper argTyper{*env, *sigs, *retParamIdxs, *diags, polyTargets};
                callNode.args[i]->accept(argTyper);
                if (!argTyper.ok) {
                    ok = false;
                    return;
                }
                if (argTyper.out != sig.params[i]) {
                    addDiag(*diags, "call argument type mismatch", callNode.args[i].get());
                    ok = false;
                    return;
                }
    ...
    if (callNode.callee && callNode.callee->kind == ast::NodeKind::Attribute) {
        ...
        if (at->value && at->value->kind == ast::NodeKind::Name) {
            ...
            /* setType is const-qualified; no mutable reference needed */
            if (it != sigs->end()) {
                const Sig &sig = it->second;
                if (!sig.full.empty()) {
                    std::unordered_map<std::string, size_t> nameToIdx;
                    ...
                    for (size_t i = 0; i < sig.full.size(); ++i) {
                        ...
                        if (p.isVarArg) varargIdx = i;
                            ...
                            nameToIdx[p.name] = i;
                            if (!p.isKwOnly) posIdxs.push_back(i);
                        }
                    }
                    std::vector<bool> bound(sig.full.size(), false);
                    for (size_t i = 0; i < callNode.args.size(); ++i) {
                        ExpressionTyper at2{*env, *sigs, *retParamIdxs, *diags, polyTargets, nullptr, classes};
                        callNode.args[i]->accept(at2);
                        if (!at2.ok) {
                            ok = false;
                            return;
                        }
                        if (i < posIdxs.size()) {
                            ...
                            if (sig.full[varargIdx].type != ast::TypeKind::NoneType && at2.out != sig.full[varargIdx].
                                type) {
                                addDiag(*diags, "*args element type mismatch", callNode.args[i].get());
                                ok = false;
                                return;
                            ...
                            addDiag(*diags,
                                    std::string("arity mismatch calling function: ") + (
                                        nameNode->id + std::string(".__call__")), &callNode);
                            ok = false;
                            return;
                        }
                    }
                    for (const auto &kw: callNode.keywords) {
                        ...
                        ExpressionTyper kt{*env, *sigs, *retParamIdxs, *diags, polyTargets, nullptr, classes};
                        if (kw.value) kw.value->accept(kt);
                        if (!kt.ok) {
                            ok = false;
                            return;
                        }
                        if (kt.out != sig.full[pidx].type) {
                            addDiag(*diags, std::string("keyword argument type mismatch: ") + kw.name, &callNode);
                            ok = false;
                            return;
                        }
                ...
                if (sig.params.size() != callNode.args.size()) {
                    addDiag(*diags, std::string("arity mismatch calling function: ") + key, &callNode);
                    ok = false;
                    return;
                }
                for (size_t i = 0; i < callNode.args.size(); ++i) {
            ...
            if (polyTargets.attrs != nullptr) {
                ...
                if (ita != polyTargets.attrs->end() && !ita->second.empty()) {
                    out = ast::TypeKind::NoneType;
                    callNode.setType(out);
                    return;
                }
