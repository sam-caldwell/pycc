...
void ExpressionTyper::visit(const ast::SetComp &sc) {
    ...
    auto inferElemMask = [&](const ast::Expr *it) -> uint32_t {
        ...
        if (it->kind == ast::NodeKind::ListLiteral) {
            uint32_t em = 0U;
            for (const auto *lst = static_cast<const ast::ListLiteral *>(it); const auto &el: lst->elements) {
                if (!el) continue;
    ...
    std::function<void(const ast::Expr *, uint32_t)> bindTarget = [&](const ast::Expr *tgt, uint32_t elemMask) {
        ...
        if (tgt->kind == ast::NodeKind::Name) {
            ...
            uint32_t m = elemMask;
            if (m == 0U) m = TypeEnv::maskForKind(ast::TypeKind::Int);
    ...
    for (const auto &f: sc.fors) {
        ...
        for (const auto &g: f.ifs) {
            ...
            if (!typeIsBool(et.out)) {
                addDiag(*diags, "set comprehension guard must be bool", g.get());
                ok = false;
                return;
            }
        }
