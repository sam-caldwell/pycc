 ...
 * Purpose: Type list literal by visiting children and setting canonical.
 */
...
void ExpressionTyper::visit(const ast::ListLiteral& n) {
  ...
  if (!expr::handleListLiteral(n, out, outSet, visitChild)) { ok = false; }
}
