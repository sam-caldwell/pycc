/**
 * @file
 * @brief handleListComp: Type-check list comprehensions via a local TypeEnv.
 */
...
bool handleListComp(const ast::ListComp& lc,
    ...
    auto listElemMask = [&](const ast::ListLiteral* lst) -> uint32_t {
        ...
        for (const auto& el : lst->elements) {
            ...
            el->accept(et); if (!et.ok) { ok = false; return 0U; }
    ...
    auto inferElemMask = [&](const ast::Expr* it) -> uint32_t {
        ...
        if (it->kind == ast::NodeKind::ListLiteral)
            return listElemMask(static_cast<const ast::ListLiteral*>(it));
        return 0U;
    };
    ...
    auto computePerIndex = [&](const ast::TupleLiteral* tp, const ast::Expr* iter, int parentIdx) -> std::vector<uint32_t> {
        ...
        if (!iter || iter->kind != ast::NodeKind::ListLiteral) return perIndex;
    ...
    for (const auto& f : lc.fors) {
        ...
        for (const auto& g : f.ifs) {
            ...
            if (!typeIsBool(et.out)) { addDiag(diags, "list comprehension guard must be bool", g.get()); ok = false; return true; }
        }
    }
    if (lc.elt) { ExpressionTyper et{local, sigs, retParamIdxs, diags, polyTargets, outers, classes}; lc.elt->accept(et); if (!et.ok) { ok = false; return true; } }
