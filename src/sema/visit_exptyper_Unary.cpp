...
void ExpressionTyper::visit(const ast::Unary &unaryNode) {
    ...
    if (operandExpr == nullptr) {
        addDiag(*diags, "null operand", &unaryNode);
        ok = false;
        return;
    }
    ...
    if (unaryNode.op == ast::UnaryOperator::Neg) {
        if (isSubset(maskVal, iMask)) {
            out = ast::TypeKind::Int;
            outSet = iMask;
        } else if (isSubset(maskVal, fMask)) {
            out = ast::TypeKind::Float;
            outSet = fMask;
        } else {
            addDiag(*diags, "unary '-' requires int or float", &unaryNode);
            ok = false;
            return;
        }
        unaryNode.setType(out);
        if (unaryNode.operand) {
            const auto &can = unaryNode.operand->canonical();
            if (can) { unaryNode.setCanonicalKey("u:neg:(" + *can + ")"); }
        }
        return;
    }
    if (unaryNode.op == ast::UnaryOperator::BitNot) {
        if (!isSubset(maskVal, iMask)) {
            addDiag(*diags, "bitwise '~' requires int", &unaryNode);
            ok = false;
            return;
        }
        out = ast::TypeKind::Int;
        ...
        if (unaryNode.operand) {
            const auto &can = unaryNode.operand->canonical();
            if (can) { unaryNode.setCanonicalKey("u:bitnot:(" + *can + ")"); }
        }
        return;
    }
    // 'not'
    if (!isSubset(maskVal, bMask)) {
        addDiag(*diags, "'not' requires bool", &unaryNode);
        ok = false;
        return;
    }
    ...
    unaryNode.setType(out);
    if (unaryNode.operand) {
        const auto &can2 = unaryNode.operand->canonical();
        if (can2) { unaryNode.setCanonicalKey("u:not:(" + *can2 + ")"); }
    }
}
