...
namespace {
struct ScanVisitor : public ast::VisitorBase {
  ...
  void addTarget(const ast::Expr* e) {
    ...
    switch (e->kind) {
      ...
      case ast::NodeKind::TupleLiteral: {
        ...
        for (const auto& el : t->elements) { if (el) addTarget(el.get()); }
        break;
      }
      case ast::NodeKind::ListLiteral: {
        const auto* l = static_cast<const ast::ListLiteral*>(e);
        for (const auto& el : l->elements) { if (el) addTarget(el.get()); }
        break;
      }
  ...
  void visit(const ast::WhileStmt& ws) override {
    for (const auto& s : ws.thenBody) if (s) s->accept(*this);
    for (const auto& s : ws.elseBody) if (s) s->accept(*this);
  }
  void visit(const ast::TryStmt& ts) override {
    for (const auto& s : ts.body) if (s) s->accept(*this);
    for (const auto& h : ts.handlers) if (h) for (const auto& s : h->body) if (s) s->accept(*this);
    for (const auto& s : ts.orelse) if (s) s->accept(*this);
    for (const auto& s : ts.finalbody) if (s) s->accept(*this);
  }
