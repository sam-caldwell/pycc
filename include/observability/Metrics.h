...
namespace pycc::obs {
    ...
    public:
        ...
        const std::optional<AstGeometry> &astGeometry() const {
            return geom_;
        }
    ...
    public:
        void setOptimizerStat(const std::string &key, const uint64_t value) {
            optimizerStats_[key] = value;
        }

        const std::unordered_map<std::string, uint64_t> &optimizerStats() const {
            return optimizerStats_;
        }

        void incOptimizerBreakdown(const std::string &pass, const std::string &key, const uint64_t delta) {
            optimizerBreakdown_[pass][key] += delta;
        }

        const auto &optimizerBreakdown() const {
            return optimizerBreakdown_;
        }

        // Generic counters/gauges for observability
        void incCounter(const std::string &key, const uint64_t delta = 1) {
            counters_[key] += delta;
        }

        void setCounter(const std::string &key, const uint64_t value) {
            counters_[key] = value;
        }

        void setGauge(const std::string &key, const uint64_t value) {
            gauges_[key] = value;
        }
