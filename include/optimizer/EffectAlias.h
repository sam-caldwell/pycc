        ...
        static bool isPureExpr(const ast::Expr *e) {
            ...
            switch (e->kind) {
                ...
                case NK::Subscript: {
                    ...
                    if (auto *s = static_cast<const ast::Subscript *>(e); !s) return false;
                ...
                case NK::TupleLiteral: {
                    ...
                    for (const auto &el: t->elements) if (!isPureExpr(el.get())) return false;
                    return true;
                }
                case NK::ListLiteral: {
                    auto *l = static_cast<const ast::ListLiteral *>(e);
                    for (const auto &el: l->elements) if (!isPureExpr(el.get())) return false;
                    return true;
                }
                default:
