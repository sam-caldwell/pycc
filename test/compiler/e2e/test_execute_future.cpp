/***
 * Name: test_execute_future
 * Purpose: Compile and run a program using __future__; verify stdout and exit code.
 */
...
static std::string slurp_future(const std::string& p) {
  std::ifstream in(p);
  std::string s((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
  return s;
}

TEST(ExecuteFuture, StdoutAndExit) {
  ...
  const auto srcPath = std::filesystem::weakly_canonical(demosDir / "e2e_future.py").string();
  ...
  fs::path outBin = outDir / "e2e_future";
  ...
  if (rc != 0) { GTEST_SKIP() << "pycc failed to compile __future__ demo"; return; }
  fs::path outTxt = outDir / "out_future.txt";
  ...
  EXPECT_EQ(rc, 0);
#endif
  auto out = slurp_future(outTxt.string());
  EXPECT_EQ(out, std::string("FUTURE_OK\\n"));
}
