/***
 * Name: test_execute_datetime
 * Purpose: Compile and run a program using datetime; verify stdout and exit code.
 */
...
static std::string slurp_dt(const std::string& p) {
  std::ifstream in(p);
  std::string s((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
  return s;
}

TEST(ExecuteDatetime, StdoutAndExit) {
  ...
  const auto srcPath = std::filesystem::weakly_canonical(demosDir / "e2e_datetime.py").string();
  ...
  fs::path outBin = outDir / "e2e_datetime";
  ...
  if (rc != 0) { GTEST_SKIP() << "pycc failed to compile datetime demo"; return; }
  fs::path outTxt = outDir / "out_datetime.txt";
  ...
  EXPECT_EQ(rc, 0);
#endif
  auto out = slurp_dt(outTxt.string());
  EXPECT_EQ(out, std::string("DATETIME_OK\\n"));
}
