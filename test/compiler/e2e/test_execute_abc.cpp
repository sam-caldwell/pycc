/***
 * Name: test_execute_abc
 * Purpose: Compile and run a program using _abc; verify stdout and exit code.
 */
...
static std::string slurp_abc(const std::string& p) {
  std::ifstream in(p);
  std::string s((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
  return s;
}

TEST(ExecuteAbc, StdoutAndExit) {
  ...
  const auto srcPath = std::filesystem::weakly_canonical(demosDir / "e2e_abc.py").string();
  ...
  if (rc != 0) { GTEST_SKIP() << "pycc failed to compile _abc demo"; return; }
  fs::path outTxt = outDir / "out_abc.txt";
  ...
  EXPECT_EQ(rc, 0);
#endif
  auto out = slurp_abc(outTxt.string());
  EXPECT_EQ(out, std::string("ABC_OK\\n"));
}
