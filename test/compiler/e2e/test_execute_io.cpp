/***
 * Name: test_execute_io
 * Purpose: Compile and run a program using io; verify stdout and exit code.
 */
...
static std::string slurp_io(const std::string& p) {
  std::ifstream in(p);
  std::string s((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
  return s;
}

TEST(ExecuteIO, StdoutAndExit) {
  ...
  const auto srcPath = std::filesystem::weakly_canonical(demosDir / "e2e_io.py").string();
  ...
  if (rc != 0) { GTEST_SKIP() << "pycc failed to compile io demo"; return; }
  fs::path outTxt = outDir / "out_io.txt";
  ...
  EXPECT_EQ(rc, 0);
#endif
  auto out = slurp_io(outTxt.string());
  EXPECT_EQ(out, std::string("IO_OK\\n"));
}
