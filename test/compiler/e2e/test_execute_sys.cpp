/***
 * Name: test_execute_sys
 * Purpose: Compile and run a program using sys; verify stdout and exit code.
 */
...
static std::string slurp_sys(const std::string& p) {
  std::ifstream in(p);
  std::string s((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
  return s;
}

TEST(ExecuteSys, StdoutAndExit) {
  ...
  const auto srcPath = std::filesystem::weakly_canonical(demosDir / "e2e_sys.py").string();
  ...
  if (rc != 0) { GTEST_SKIP() << "pycc failed to compile sys demo"; return; }
  fs::path outTxt = outDir / "out_sys.txt";
  ...
  EXPECT_EQ(rc, 0);
#endif
  auto out = slurp_sys(outTxt.string());
  EXPECT_EQ(out, std::string("SYS_OK\\n"));
}
