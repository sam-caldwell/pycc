/***
 * Name: test_execute_argparse
 * Purpose: Compile and run a program using argparse; verify stdout and exit code.
 */
...
static std::string slurp_ap(const std::string& p) {
  std::ifstream in(p);
  std::string s((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
  return s;
}

TEST(ExecuteArgparse, StdoutAndExit) {
  ...
  const auto srcPath = std::filesystem::weakly_canonical(demosDir / "e2e_argparse.py").string();
  ...
  fs::path outBin = outDir / "e2e_argparse";
  ...
  if (rc != 0) { GTEST_SKIP() << "pycc failed to compile argparse demo"; return; }
  fs::path outTxt = outDir / "out_argparse.txt";
  ...
  EXPECT_EQ(rc, 0);
#endif
  auto out = slurp_ap(outTxt.string());
  EXPECT_EQ(out, std::string("ARGPARSE_OK\\n"));
}
