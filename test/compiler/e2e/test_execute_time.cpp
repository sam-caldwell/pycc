/***
 * Name: test_execute_time
 * Purpose: Compile and run a program using time; verify stdout and exit code.
 */
...
TEST(ExecuteTime, StdoutAndExit) {
  ...
  const auto srcPath = std::filesystem::weakly_canonical(demosDir / "e2e_time.py").string();
  ...
  fs::path outBin = outDir / "e2e_time";
  ...
  if (rc != 0) { GTEST_SKIP() << "pycc failed to compile time demo"; return; }
  fs::path outTxt = outDir / "out_time.txt";
  ...
  EXPECT_EQ(rc, 0);
#endif
  auto out = slurp(outTxt.string());
  EXPECT_EQ(out, std::string("TIME_OK\\n"));
}
