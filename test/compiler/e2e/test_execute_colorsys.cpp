/***
 * Name: test_execute_colorsys
 * Purpose: Compile and run a program using colorsys; verify stdout and exit code.
 */
...
static std::string slurp_colorsys(const std::string& p) {
  std::ifstream in(p);
  std::string s((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
  return s;
}

TEST(ExecuteColorsys, StdoutAndExit) {
  ...
  const auto srcPath = std::filesystem::weakly_canonical(demosDir / "e2e_colorsys.py").string();
  ...
  fs::path outBin = outDir / "e2e_colorsys";
  ...
  if (rc != 0) { GTEST_SKIP() << "pycc failed to compile colorsys demo"; return; }
  fs::path outTxt = outDir / "out_colorsys.txt";
  ...
  EXPECT_EQ(rc, 0);
#endif
  auto out = slurp_colorsys(outTxt.string());
  EXPECT_EQ(out, std::string("COLORSYS_OK\\n"));
}
