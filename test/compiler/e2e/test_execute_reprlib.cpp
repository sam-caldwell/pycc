/***
 * Name: test_execute_reprlib
 * Purpose: Compile and run a program using reprlib; verify stdout and exit code.
 */
...
static std::string slurp_repr(const std::string& p) {
  std::ifstream in(p);
  std::string s((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
  return s;
}

TEST(ExecuteReprlib, StdoutAndExit) {
  ...
  const auto srcPath = std::filesystem::weakly_canonical(demosDir / "e2e_reprlib.py").string();
  ...
  fs::path outBin = outDir / "e2e_reprlib";
  ...
  if (rc != 0) { GTEST_SKIP() << "pycc failed to compile reprlib demo"; return; }
  fs::path outTxt = outDir / "out_reprlib.txt";
  ...
  EXPECT_EQ(rc, 0);
#endif
  auto out = slurp_repr(outTxt.string());
  EXPECT_EQ(out, std::string("REPRLIB_OK\\n"));
}
