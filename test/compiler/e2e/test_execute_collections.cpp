/***
 * Name: test_execute_collections
 * Purpose: Compile and run a program using collections; verify stdout and exit code.
 */
...
static std::string slurp_collections(const std::string& p) {
  std::ifstream in(p);
  std::string s((std::istreambuf_iterator<char>(in)), std::istreambuf_iterator<char>());
  return s;
}

TEST(ExecuteCollections, StdoutAndExit) {
  ...
  const auto srcPath = std::filesystem::weakly_canonical(demosDir / "e2e_collections.py").string();
  ...
  fs::path outBin = outDir / "e2e_collections";
  ...
  if (rc != 0) { GTEST_SKIP() << "pycc failed to compile collections demo"; return; }
  fs::path outTxt = outDir / "out_collections.txt";
  ...
  EXPECT_EQ(rc, 0);
#endif
  auto out = slurp_collections(outTxt.string());
  EXPECT_EQ(out, std::string("COLLECTIONS_OK\\n"));
}
