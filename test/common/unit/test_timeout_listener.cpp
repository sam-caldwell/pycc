// GTest per-test timeout listener (300s default)
...
class TimeoutListener : public ::testing::EmptyTestEventListener {
 public:
  ...
  void OnTestStart(const ::testing::TestInfo& test_info) override {
    ...
    worker_ = std::thread([this, name = std::string(test_info.test_suite_name()) + "." + test_info.name()]() {
...
class TimeoutEnv : public ::testing::Environment {
 public:
  void SetUp() override {
    int secs = 300;
    if (const char* env = std::getenv("PYCC_GTEST_TIMEOUT_SECS")) {
      int v = std::atoi(env); if (v > 0) secs = v;
    }
    ::testing::UnitTest::GetInstance()->listeners().Append(new TimeoutListener(secs));
  }
};

// Register the environment globally
auto* const kTimeoutEnvReg = ::testing::AddGlobalTestEnvironment(new TimeoutEnv());
} // namespace
