...
TEST(OptimizerMetrics, FoldsReported) {
  const char* src = "../Testing/metrics_tmp_cf.py";
  {
    std::ofstream out(src);
    out << "def main() -> int:\n";
    out << "  return 2 + 3\n";
  }
  const char* cmd = "./pycc --opt-const-fold --metrics-json -S -o ../Testing/out_cf ../Testing/metrics_tmp_cf.py > ../Testing/metrics_cf.json 2>/dev/null";
  int rc = std::system(cmd);
  ASSERT_EQ(rc, 0);
  auto js = readFileAll("../Testing/metrics_cf.json");
